...
const SignInPopup: React.FC = () => {
    ...
    function closeModalSign(){
        modalRefSign.current?.close()
    }
    return(
        <>
            <button className="header__enter" onClick={openModalSign}>Войти</button>
            <dialog ref={modalRefSign} className="modal modal_auth">

                <form className="modal__content" method="dialog">
                    <button onClick={closeModalSign} className="modal__content-close" id="closeAuthBtn">
                        {/* <img src="../image/crossRed.svg" alt="закрыть"/> */}
                    ...
                    <p className="modal__content-text">Нет аккаунта?<RegistrationPopup/></p>
                </form>
            </dialog>
        </>
    )
