...
type UpdatePatientInfoProps ={
    id: number,
    first_name_edit: string | undefined,
    last_name_edit: string | undefined,
    patronymic_edit: string | undefined,
    birth_date_edit: string | undefined,
    region_edit: string | undefined,
...
export default function UpdatePatientInfo({id, first_name_edit, last_name_edit, patronymic_edit, region_edit, diagnosis_edit, diagnosis_comment_edit, operation_comment_edit, chemoterapy_comment_edit}:UpdatePatientInfoProps){
    
  const dispatch = useAppDispatch();
  const navigate = useNavigate();
  const urlParams = useParams();
    const patient = [last_name_edit, first_name_edit, patronymic_edit].join(' ')
    const [open, setOpen] = useState(false);
    const [error, setError] = useState('');
    const [FIO, setFIO] = useState(patient);
    const [birth_date, setDate] = useState('');
    const [region, setRegion] = useState(region_edit);
    const authorizationStatus = useAppSelector(getAuthorizationStatus);
    const dateTime = new Date();
  
    const today = new Date(getAltDateFor(dateTime));
    const birth = new Date(birth_date);
    const diagnos = diagnosis_edit
  
  
    const [diagnoses, setDiagnoses] = useState(diagnos);
    const handleSubmit = (evt: FormEvent<HTMLFormElement>) => {
        ...
        if(authorizationStatus !== AuthorizationStatus.Auth){
          ...
          dispatch(UpdatePatientsAction({
            ...
            region: region,
            diagnosis: diagnoses,
            diagnosis_comment: diagnosis_comment_edit,
            operation_comment: operation_comment_edit,
            chemoterapy_comment: chemoterapy_comment_edit
          }));
          handleClose();
